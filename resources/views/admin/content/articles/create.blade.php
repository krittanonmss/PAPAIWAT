<x-layouts.admin title="สร้างบทความ" header="สร้างบทความ">
    <div class="space-y-6 text-white">
            ...
            <div class="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-2xl">
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-400">
                        เพิ่มบทความใหม่ พร้อมข้อมูลเนื้อหา การเผยแพร่ SEO หมวดหมู่ แท็ก รูปภาพ และบทความที่เกี่ยวข้อง
                    </p>
                </div>
        ...
        <form
            id="article-form"
            method="POST"
            action="{{ route('admin.content.articles.store') }}"
            class="space-y-6"
        >
            @csrf
            ...
            <div x-data="{ activeTab: 'content' }" :class="`article-studio-tab-${activeTab}`" class="space-y-6">
                @include('admin.content.articles.partials._template_preview_panel', [
                    'article' => new \App\Models\Content\Article\Article(),
                ])
