<x-layouts.admin :title="'เพิ่มรายการเมนู'">
    <div class="space-y-6 text-white">
        <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-slate-950/30 backdrop-blur">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <h1 class="mt-1 text-2xl font-bold text-white">เพิ่มรายการใน {{ $menu->name }}</h1>
                    <p class="mt-2 text-sm text-slate-400">สร้างลิงก์ เมนูย่อย หรือหัวข้อกลุ่มได้จากหน้าเดียว</p>
                </div>
        ...
        <form method="POST" action="{{ route('admin.content.menu-items.store', $menu) }}" class="space-y-6">
            @csrf
            @include('admin.content.layout.menu-items._form')
            ...
            <div class="sticky bottom-0 z-20 -mx-2 rounded-t-3xl border border-white/10 bg-slate-950/90 px-4 py-4 shadow-2xl shadow-slate-950 backdrop-blur">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-950/40 transition hover:bg-blue-500">เพิ่มรายการเมนู</button>
                </div>
            </div>
        </form>
    </div>
