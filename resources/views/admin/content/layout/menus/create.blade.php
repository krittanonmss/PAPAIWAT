<x-layouts.admin :title="'Create Menu'" header="สร้างเมนู">
    <div class="space-y-6 text-white">
        ...
        <div class="overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950 shadow-xl shadow-slate-950/30">
            <div class="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-2xl">
                    ...
                    <h1 class="text-2xl font-bold text-white">สร้างเมนู</h1>

                    <p class="mt-2 text-sm leading-6 text-slate-400">
                        สร้างกลุ่มเมนูสำหรับใช้งานในตำแหน่งต่าง ๆ ของเว็บไซต์ เช่น Header, Footer หรือ Sidebar
                    </p>
                </div>
        ...
        <form
            method="POST"
            action="{{ route('admin.content.menus.store') }}"
            class="space-y-6"
        >
            @csrf
            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <div class="space-y-6">
                    <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-slate-950/30 backdrop-blur">
                        @include('admin.content.layout.menus._form')
                    </div>
                </div>
                    ...
                    <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-xl shadow-slate-950/30 backdrop-blur">
                        ...
                        <div class="mt-4 space-y-3">
                            <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                                <p class="text-sm font-medium text-slate-200">การตั้งชื่อ</p>
                                <p class="mt-1 text-xs leading-5 text-slate-500">
                                    ควรตั้งชื่อให้สื่อถึงตำแหน่ง เช่น Main Header หรือ Footer Links
                                </p>
                            </div>

                            <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                                <p class="text-sm font-medium text-slate-200">Slug และตำแหน่ง</p>
                                <p class="mt-1 text-xs leading-5 text-slate-500">
                                    ตรวจสอบ slug และ location key ให้ตรงกับตำแหน่งที่ต้องการใช้งาน
                                </p>
                            </div>

                            <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                                <p class="text-sm font-medium text-slate-200">การใช้งาน</p>
                                <p class="mt-1 text-xs leading-5 text-slate-500">
                                    สามารถนำเมนูไปผูกกับ Layout หรือ Page ได้ภายหลัง
                                </p>
                            </div>
                        </div>
                    </div>
