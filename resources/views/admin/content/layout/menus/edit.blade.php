<x-layouts.admin :title="'Edit Menu'" header="แก้ไขเมนู">
    <div class="space-y-6 text-white">
        ...
        <div class="overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950 shadow-xl shadow-slate-950/30">
            <div class="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-2xl">
                    ...
                    <h1 class="text-2xl font-bold text-white">แก้ไขเมนู</h1>
        ...
        <form
            method="POST"
            action="{{ route('admin.content.menus.update', $menu) }}"
            class="space-y-6"
        >
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <div class="space-y-6">
                    <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-slate-950/30 backdrop-blur">
                        @include('admin.content.layout.menus._form', ['menu' => $menu])
                    </div>
                </div>
                    ...
                    <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-xl shadow-slate-950/30 backdrop-blur">
                        ...
                        <div class="mt-4 space-y-3">
                            <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                                <p class="text-sm font-medium text-slate-200">ข้อมูลเมนู</p>
                                <p class="mt-1 text-xs leading-5 text-slate-500">
                                    แก้ไขชื่อ slug ตำแหน่ง และรายละเอียดของกลุ่มเมนู
                                </p>
                            </div>
                            ...
                            <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                                ...
                                <p class="mt-1 text-xs leading-5 text-slate-500">
                                    ตรวจสอบสถานะ default และ location key ก่อนบันทึก
                                </p>
                            </div>

                            <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                                <p class="text-sm font-medium text-slate-200">รายการเมนูย่อย</p>
                                <p class="mt-1 text-xs leading-5 text-slate-500">
                                    รายการเมนูภายในกลุ่มนี้จัดการได้จากหน้ารายละเอียดเมนู
                                </p>
                            </div>
                        </div>
                    </div>
