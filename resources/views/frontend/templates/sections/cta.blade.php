...
<section class="px-4 py-14" style="@include('frontend.templates.sections._background')">
    <div data-section-card data-section-card-padding class="mx-auto max-w-7xl rounded-3xl border border-blue-300/20 bg-blue-500/10 p-8 text-center shadow-xl shadow-blue-950/20">
        ...
        @if(($showPrimaryButton && !empty($content['primary_label']) && !empty($content['primary_url'])) || ($showSecondaryButton && !empty($content['secondary_label']) && !empty($content['secondary_url'])))
            <div class="mt-6 flex flex-wrap justify-center gap-3">
                ...
                <a href="{{ $content['primary_url'] }}" data-section-button class="inline-flex rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-500">{{ $content['primary_label'] }}</a>
                @endif
                @if($showSecondaryButton && !empty($content['secondary_label']) && !empty($content['secondary_url']))
                    <a href="{{ $content['secondary_url'] }}" data-section-button class="inline-flex rounded-2xl border border-white/10 bg-white/[0.06] px-5 py-3 text-sm font-semibold text-slate-200 transition hover:bg-white/10 hover:text-white">{{ $content['secondary_label'] }}</a>
                @endif
            </div>
        @endif
    </div>
